...
import { intOpt, positiveSafeIntOpt, required } from '../cli-args.mjs';
...
export function createWebPeerActions(deps = {}) {
  ...
  function webPeerHeartbeat(projectCtx, peer, input = {}) {
    webMutationPeer(peer, input);
    const ttlOverride = input.ttl === undefined
      ? null
      : positiveSafeIntOpt(input, 'ttl', defaultLockTtl);
  ...
  function webPeerLockAcquire(projectCtx, peer, input = {}) {
    ...
    const ttl = positiveSafeIntOpt(input, 'ttl', defaultLockTtl);
    ...
    try {
      ...
      const outcome = runOptimisticEvidenceMutation(db, {
        ...
        const conflict = subject.locks.find((row) => {
          if (row.owner === peer || !locksConflict(row, requested)) return false;
          const ownerEvidence = evidenceByOwner.get(row.owner)?.state;
          if (graceActive && Number(row.expires_at) <= t && ownerEvidence === 'dead') return false;
          return graceActive || Number(row.expires_at) > t || ownerEvidence === 'live';
        });
        if (conflict) {
