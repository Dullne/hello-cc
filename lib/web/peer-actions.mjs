...
export function createWebPeerActions(deps = {}) {
  ...
  function webPeerHeartbeat(projectCtx, peer, input = {}) {
    ...
    try {
      ...
      if (renewLocks) {
        ...
        const evidenceLive = peerEvidenceFromDb(db, projectCtx, peer).state === 'live';
        if (ttlOverride !== null) {
  ...
  function webPeerLockAcquire(projectCtx, peer, input = {}) {
    ...
    try {
      ...
      const lock = runOptimisticEvidenceMutation(db, {
        ...
        const graceActive = clockGraceSuppressed(t, subject.graceUntil);
