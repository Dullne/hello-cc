...
export function webIndexHtml({ nonce } = {}) {
...
<body>
  ...
  <script${inlineScriptNonce}>
    ...
    sessionStorage.removeItem('hcc_logged_out');
    ...
    function resizeTerm() {
      ...
      if (ws && ws.readyState === WebSocket.OPEN && actionToken) {
        terminalLastResizeAt = Date.now();
        ws.send(JSON.stringify({ type: 'resize', cols, rows, action_token: actionToken }));
      }
    }
    ...
    function sendTerminalInput(data) {
      if (!active || !ws) return;
      const actionToken = sessionActionTokens.get(active) || '';
      if (ws.readyState === WebSocket.OPEN && actionToken) {
        ws.send(JSON.stringify({ type: 'input', data, action_token: actionToken }));
        return;
      }
      if (ws.readyState !== WebSocket.CONNECTING && ws.readyState !== WebSocket.OPEN) return;
      const queued = (pendingTerminalInput.get(active) || '') + String(data || '');
      pendingTerminalInput.set(active, queued.slice(-65536));
    }

    function flushPendingTerminalInput(id, socket) {
      const data = pendingTerminalInput.get(id) || '';
      const actionToken = sessionActionTokens.get(id) || '';
      if (!data || active !== id || ws !== socket || socket.readyState !== WebSocket.OPEN || !actionToken) return;
      pendingTerminalInput.delete(id);
    ...
    function openWs(id) {
      ...
      socket.onmessage = (event) => {
        ...
        if (msg.type === 'snapshot') {
          ...
          resizeTerm();
          flushPendingTerminalInput(id, socket);
        }
        if (msg.type === 'data') {
      ...
      socket.onclose = (event) => {
        ...
        if (event.code === 4001 || sessionStorage.getItem('hcc_logged_out') === '1') {
          connText('signed out');
          return;
        }
    ...
    async function runPeerAction(action) {
      ...
      const payload = action === 'register'
        ...
        : action === 'heartbeat'
          ? { renew_locks: true }
          : {};
      if (!readOnly) payload.action_token = info.actionToken;
    ...
    document.getElementById('logoutBtn').addEventListener('click', async () => {
      ...
      // Stop reconnect only after the server confirms revocation. A failed
      // logout must not poison a later authenticated page in this tab.
      sessionStorage.setItem('hcc_logged_out', '1');
      location.replace('/');
    });
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; background: #101418; color: #e8eaed; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
    .card { background: #1a2027; border: 1px solid #2c343d; border-radius: 10px; padding: 28px; width: 320px; }
    h1 { font-size: 18px; margin: 0 0 16px; }
    label { display: block; font-size: 13px; margin-bottom: 6px; color: #aab3bd; }
    input { width: 100%; box-sizing: border-box; padding: 9px 10px; border-radius: 6px; border: 1px solid #39424c; background: #101418; color: #e8eaed; font-size: 14px; }
    button { margin-top: 16px; width: 100%; padding: 9px; border-radius: 6px; border: 0; background: #40c4aa; color: #06211b; font-weight: 600; font-size: 14px; cursor: pointer; }
    button:hover { background: #57d4bc; }
    .err { color: #ff8b8b; font-size: 13px; margin-top: 10px; min-height: 18px; }
  </style>
</head>
<body>
  <div class="card">
    ...
    <input id="tok" type="password" autocomplete="off" placeholder="paste the ?token= value" autofocus>
  ...
  <script${inlineScriptNonce}>
    ...
    async function submit() {
      ...
      if (res.ok) { location.href = '/'; } else { err.textContent = 'Invalid token.'; }
    }
