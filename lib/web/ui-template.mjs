...
<body>
  ...
  <script${inlineScriptNonce}>
    ...
    // Per-session action tokens, delivered only via the terminal WebSocket
    // snapshot frame (never via the session list). Required on input frames.
    ...
    function openWs(id) {
      ...
      ws = new WebSocket(proto + '://' + location.host + '/ws/terminal/' + encodeURIComponent(id) + requestQuery({ api_version: runtimeApiVersion }));
      ws.onopen = () => {
        resizeTerm();
        connText('attached');
      };
    ...
    function sendLine(text) {
      ...
      ws.send(JSON.stringify({ type: 'input', data: text + '\\r', action_token: sessionActionTokens.get(active) || '' }));
    }
