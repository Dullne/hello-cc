...
const LAUNCH_ENV_IGNORED_KEYS = new Set([
  ...
  'HCC_PEER',
  'HCC_ROOT',
  'HCC_RUNTIME_LOCAL_ONLY',
  'HCC_SKIP_SHIM_INSTALL',
  PROVIDER_STATE_ENV,
  'HCC_SHIM_ENSURED',
  'HCC_SHIM_NO_ATTACH',
  'HCC_WEB_TOKEN',
...
export function childSessionEnv(extra = {}, baseEnv = process.env) {
  ...
  delete env.HCC_RUNTIME_LOCAL_ONLY;
  return env;
}
...
export function providerRestartReason({ providerState, ownerEvidence, paneCommand } = {}) {
  if (ownerEvidence?.state !== 'dead') return null;
  if (providerState === 'exited') return 'provider_exited';
  if (!providerState && isLikelyShellCommand(paneCommand)) return 'provider_fallback_shell';
  return null;
}
