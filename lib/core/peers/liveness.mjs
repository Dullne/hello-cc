...
// A peer explicitly marked dead is never "active", regardless of how recently
// last_seen_at was written. Death paths historically bumped last_seen_at to now,
// which made a just-killed owner read as active for a full TTL window (hb-01).
const DEAD_PEER_STATUSES = new Set(['exited', 'detached']);
...
export function taskOwnerLiveness(task, peers, locks, t = nowSec(), activePeerTtl = DEFAULT_ACTIVE_PEER_TTL) {
  ...
  const ownerAge = ownerRow
    ...
    : null;
  const ownerDead = Boolean(ownerRow && DEAD_PEER_STATUSES.has(ownerRow.status));
  const ownerActive = Boolean(ownerRow && !ownerDead && Number.isFinite(ownerAge) && ownerAge <= activePeerTtl);
