...
function providerCompatible(existing, binding) {
  if (!existing?.provider || !binding?.provider) return true;
  return existing.provider === binding.provider;
}

function providerSessionResumeMode(mode) {
  return new Set(['detected', 'resume', 'session', 'named']).has(mode);
}

export function mergePeerBinding(existing, binding) {
  let next = mergeRuntimeBinding(existing, binding);
  if (!existing || bindingHasProviderSession(next) || !bindingHasProviderSession(existing)) return next;
  if (!providerCompatible(existing, next)) return next;

  next = {
    ...next,
    provider: next.provider || existing.provider,
    provider_session_id: existing.provider_session_id || null,
    provider_session_name: existing.provider_session_name || null
  };
  if (providerSessionResumeMode(existing.resume_mode) && !providerSessionResumeMode(next.resume_mode)) {
    next.resume_mode = existing.resume_mode;
    next.resume_arg = existing.resume_arg || next.resume_arg || null;
  }
  return next;
}
