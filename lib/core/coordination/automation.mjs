...
import { classifyPeerActivity } from '../peers/evidence.mjs';
...
export function deriveAutomation(snapshot, peer = null, opts = {}, config = {}) {
  ...
  const peerId = peer || '';
  const graceActive = Boolean(snapshot.clock_grace_active);
  ...
  const requestedLocks = resources.map((resource) => {
    ...
    const lock = (snapshot.locks || []).find((row) => locksConflict(row, requested));
    const exact = (snapshot.locks || []).find((row) => lockBaseResource(row) === requested.base_resource && lockScope(row) === requested.scope);
  ...
  const automation = {
    ...
    peer: peerId ? {
      ...
      active: Boolean(peerRow && classifyPeerActivity(peerRow, { activePeerTtl, graceActive }).active),
