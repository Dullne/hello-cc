...
export function createTaskStore(deps = {}) {
  ...
  function takeoverPolicyDetails(db, row, peer, { policy = 'blocked-or-stale', staleAfter = activePeerTtl } = {}) {
    ...
    const ownerRow = previousOwner ? db.prepare('SELECT id, status, last_seen_at FROM peers WHERE id = ?').get(previousOwner) : null;
    const ownerAge = ownerRow ? now() - Number(ownerRow.last_seen_at || 0) : null;
    // A peer explicitly marked dead is stale immediately, even if a death path
    // bumped its last_seen_at to now (hb-01) — otherwise `takeover --policy stale`
    // is wrongly rejected for a task owned by a just-killed peer.
    const ownerDead = Boolean(ownerRow && ['exited', 'detached'].includes(ownerRow.status));
    const ownerStale = Boolean(previousOwner && previousOwner !== peer && (!ownerRow || ownerDead || ownerAge > staleAfter));
  ...
  function takeOverTaskForPeer(db, peer, id, { reason, policy = 'blocked-or-stale', staleAfter = activePeerTtl, source = null } = {}) {
    return tx(db, () => {
      const row = db.prepare('SELECT * FROM tasks WHERE id = ?').get(id);
      if (!row) throw new CliError('NOT_FOUND', `Task #${id} does not exist`);
