...
export function finalizeGcLockSubjects(db, subjects, evidenceByOwner, options = {}) {
  ...
  return tx(db, () => {
    ...
    if (!dryRun && options.beforeMutate?.({ subjects }) === false) {
      return { ...result, deferred: subjects.length, blocked: true };
    }
...
function snapshotProtectedCount(db, snapshot, category) {
  if (category !== 'handoffs') return 0;
...
export function createHistoryGcSnapshot(db, cutoff, options = {}) {
  ...
  try {
    readTx(db, () => {
      ...
      snapshot.protectedCounts = Object.fromEntries(categories.map((category) => [
        category,
        snapshotProtectedCount(db, snapshot, category)
      ]));
...
export async function runWithHistoryGcSnapshotCleanupAsync(
  db,
  snapshot,
  operation,
  context = 'History GC operation'
...
export function finalizeHistoryGcPlan(db, plan, options = {}) {
  ...
  try {
    return tx(db, () => {
      if (!options.dryRun && options.beforeMutate?.({ plan, categories }) === false) {
        return {
          applied: false,
          blocked: true,
          ...planCounts({ events: [], tasks: [], messages: [], handoffs: [] }, categories),
  ...
  const processBatches = () => {
    ...
    while (historyPlanCount(plan) > 0) {
      const result = finalizeHistoryGcPlan(db, plan, {
        ...
        beforeMutate: options.beforeMutate
      });
