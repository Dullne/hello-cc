...
export function captureLockRenewalSubject(db, { owner, now }) {
  const peer = db.prepare(`
    SELECT id, status, pid, pid_start_token, pid_command_hash, last_seen_at
    FROM peers WHERE id = ?
  `).get(owner) || null;
  ...
  const locks = db.prepare(`
    SELECT * FROM locks WHERE owner = ? ORDER BY resource ASC
  `).all(owner);
  return {
    owner,
    peer,
...
export function clockCandidatesFromLocks(subject, evidenceByOwner, owner = null) {
  return (subject.locks || [])
    .filter((lock) => Number(lock.expires_at) <= subject.observedAt && (!owner || lock.owner === owner))
    .map((lock) => ({
