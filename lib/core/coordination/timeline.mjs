...
export function payloadParticipants(payload) {
  const values = [];
  for (const key of ['peer', 'actor_peer', 'target_peer', 'from_peer', 'to_peer', 'assignee', 'owner', 'recipient', 'kept_peer', 'removed_peer', 'previous_owner']) {
...
const TIMELINE_EVENT_ALLOW = new Set([
  ...
  'peer.joined',
  'peer.auto_joined',
  'peer.start.requested',
  'peer.attach.requested',
  'peer.stop.requested',
  'peer.stopped',
  'peer.restarted',
  'provider.session.merged',
  'provider.session.rebound',
  'web.session.started',
  'web.session.exited',
  'web.session.stop_requested',
  'tmux.session.attached',
  'tmux.session.detached',
  'tmux.session.exited',
  'tmux.session.restarted',
  'tmux.session.rebound',
  'tmux.session.gc',
  'run.session.started',
  'run.session.exited'
]);
