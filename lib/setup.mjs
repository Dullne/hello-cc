...
/**
 * Append SHIM_DIR to PATH in the user's shell rc file if not already present.
...
export function installPathEntry() {
  ...
  if (content.includes('.hcc-shims')) {
    return { rcFile, shellType, alreadyPresent: true };
  }

  const line = shellType === 'fish'
    ? '\n# hello-cc shims\nset -gx PATH $HOME/.hcc-shims $PATH\n'
    : '\nexport PATH="$HOME/.hcc-shims:$PATH"  # hello-cc shims\n';
  fs.appendFileSync(rcFile, line);
  return { rcFile, shellType, alreadyPresent: false };
}
