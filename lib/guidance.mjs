...
- Acquire an advisory lock for the file, directory, module scope, or shared
  resource.
- Locks are coordination signals, not task ownership and not read barriers.
  A broad lock such as \`--resource bin/hcc.mjs\` protects the whole file, while
  scoped locks such as \`--resource bin/hcc.mjs --scope db-schema\` and
  \`--scope web-ui\` can be held by different peers at the same time.
- If another live peer holds a conflicting lock, message that peer before
  editing. When the work is in separate regions of the same file, agree on
  narrower scoped locks and wait for all related scopes to hand off or release
  before final tests, formatting, staging, or commit.
