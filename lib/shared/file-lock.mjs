...
function canonicalTarget(value, { createParent = true } = {}) {
  ...
  if (createParent) fs.mkdirSync(suppliedParent, { recursive: true, mode: 0o700 });
...
function candidatePortsForKey(key) {
  const ports = [];
  for (let index = 0; ports.length < PORT_CANDIDATE_COUNT; index += 1) {
    const digest = index === 0
      ? createHash('sha256').update(key).digest()
      : createHash('sha256').update(`hcc-file-lock-port-v1\0${index}\0${key}`).digest();
    const port = PORT_BASE + digest.readUInt32BE(0) % PORT_COUNT;
    if (!ports.includes(port)) ports.push(port);
  }
  return ports;
}

function lockIdentityForKey(key) {
  return createHash('sha256').update(`hcc-file-lock-v1\0${key}`).digest('hex');
}

function lockTargetsForLocation(location) {
  return [...new Set([location.suppliedKey, location.key])]
    .sort()
    .map((key) => ({
      identity: lockIdentityForKey(key),
      ports: candidatePortsForKey(key)
    }));
}
...
// The primary endpoint remains stable for diagnostics. Acquisition uses
// additional deterministic candidates plus an identity handshake so an
// unrelated local listener cannot impersonate this target's lock.
...
function acquireWorker(location, options, workerFactory) {
  ...
  const targets = lockTargetsForLocation(location);
...
export function createFileLock({ workerFactory = defaultWorkerFactory } = {}) {
  ...
  return function withConfiguredFileLock(
    ...
    const nonblocking = options.nonblocking === undefined ? false : options.nonblocking;
    const createParent = options.createParent === undefined ? true : options.createParent;
    ...
    if (typeof createParent !== 'boolean') {
      throw new TypeError('file lock createParent must be a boolean');
    }
    if (fn.constructor?.name === 'AsyncFunction') {
      throw new TypeError('withFileLock requires a synchronous callback');
    }

    const location = canonicalTarget(target, { createParent });
