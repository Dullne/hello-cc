...
import { applyBufferPlan, bufferPlanGcCutoffs } from './buffer-gc.mjs';
  ...
  for (let offset = 0; offset < plan.deleteEntries.length; offset += batchSize) {
    ...
    const applied = tx(db, () => {
      ...
      const observation = observeClockSafetyInTransaction(db, {
        operation: 'gc',
        gcCutoffs: bufferPlanGcCutoffs({ ...plan, deleteEntries: entries }, retentionSec),
        nowSec: observedAt
      });
      if (clockGraceSuppressed(observedAt, observation.graceUntil)) return null;
      return applyBufferPlan({
        ...plan,
        protectedEntries: [],
        unknownEntries: [],
    ...
    if (!applied) {
      result.deferred += plan.deleteEntries.length - offset;
      result.complete = false;
      result.graceActive = true;
      break;
    }
    result.deleted += applied.deleted;
    result.deferred += applied.deferred;
    if (applied.deferred > 0) result.complete = false;
