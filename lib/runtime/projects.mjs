...
function updateProjectRegistry(candidate, lockOptions = {}) {
  ...
  return withFileLock(registryPath, (lockedRegistryPath) => {
    ...
    // v1-05: a negative delta (future timestamp / clock step-back) must bypass
    // the throttle so the registry is refreshed immediately — the original
    // ageSeconds >= 0 guard already handled this correctly.
...
export function registerProject(ctx, lockOptions = {}) {
  const candidate = projectRecord(ctx);
  if (!candidate) throw new TypeError('project root is required');
  // xx-01: the throttle check lives inside the lock (canonical-domain safe, see
  // test/file-lock.test.mjs 'HOME is retargeted'). The request hot path is kept
  // off the lock by rememberProject skipping projects it already resolved.
  return updateProjectRegistry(candidate, lockOptions);
}
