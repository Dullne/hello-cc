...
export function classifyRuntimePointer(runtime, {
  ...
  const storedEvidence = runtimeIdentityEvidence(runtime);
  const stored = storedEvidence.present ? storedEvidence.identity : null;
...
function runtimeIdentityEvidence(runtime) {
  const snakeCase = Object.hasOwn(runtime || {}, 'process_identity');
  const camelCase = Object.hasOwn(runtime || {}, 'processIdentity');
  if (!snakeCase && !camelCase) return { present: false, identity: null };
  if (snakeCase && camelCase) return { present: true, identity: null };
  return {
    present: true,
    identity: snakeCase ? runtime.process_identity : runtime.processIdentity
  };
}
...
function runtimePointerMatchesCurrentProcess(runtime, {
  ...
  const storedEvidence = runtimeIdentityEvidence(runtime);
  if (!storedEvidence.present) return pidAlive(runtime?.pid);
  const stored = storedEvidence.identity;
  if (!completeProcessIdentity(stored)) return false;
  const pid = Number(runtime?.pid);
  if (!Number.isInteger(pid) || pid <= 0 || stored.pid !== pid) return false;
  return classifyRuntimePointer(runtime, { ageMs: 0, inspect }).state === 'alive';
}

function runtimeMetadataMatchesPointer(runtime, metadata) {
  if (Object.hasOwn(runtime || {}, 'product') &&
      (typeof runtime.product !== 'string' || !runtime.product ||
       metadata?.product !== runtime.product)) return false;
  const storedEvidence = runtimeIdentityEvidence(runtime);
  if (!storedEvidence.present) return true;
  const stored = storedEvidence.identity;
  const observedEvidence = runtimeIdentityEvidence(metadata);
  if (!completeProcessIdentity(stored) || !observedEvidence.present) return false;
...
export function readRuntime(ctx, opts = {}) {
  ...
  const inspectRuntimeProcessIdentity = typeof opts.inspectProcessIdentity === 'function'
  ...
  if (process.env.HCC_RUNTIME_URL) {
    return {
      ...
      tls_ca_file: process.env.HCC_RUNTIME_CA || undefined,
      source: 'env'
    };
  }
  const file = runtimePath(ctx);
  if (fs.existsSync(file)) {
    try {
      ...
      if (!runtimePointerMatchesCurrentProcess(runtime, { inspect: inspectRuntimeProcessIdentity })) {
  ...
  if (!localOnly) {
    ...
    if (global?.base_url && runtimePointerMatchesCurrentProcess(global, {
      inspect: inspectRuntimeProcessIdentity
    })) {
...
export async function probeRuntime(runtime) {
  ...
  try {
    // xx-03: probing must not hang the CLI indefinitely against a stalled runtime.
