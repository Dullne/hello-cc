...
export async function probeRuntime(runtime) {
  ...
  try {
    const response = await runtimeHttpRequest(runtime, '/api/runtime', { headers });
...
export function clearRuntime(ctx, pid = process.pid) {
  ...
  if (fs.existsSync(file)) {
    try {
      const runtime = JSON.parse(fs.readFileSync(file, 'utf8'));
      if (runtime.pid === pid) fs.rmSync(file, { force: true });
    } catch {
  ...
  try {
    ...
    if (runtime.pid === pid) fs.rmSync(globalFile, { force: true });
  } catch {
