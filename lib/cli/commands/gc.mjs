...
function withOptionalBufferDirectoryLease(directory, callback) {
  let stat;
  try {
  ...
  if (stat.isSymbolicLink()) {
    throw new CliError(
      ...
      `Buffer directory must be a real directory: ${path.resolve(directory)}`
    );
  }
  return withBufferDirectoryLease(directory, (leasedDirectory) => {
    const current = fs.lstatSync(directory);
    if (current.isSymbolicLink() || !current.isDirectory() ||
        current.dev !== stat.dev || current.ino !== stat.ino ||
        fs.realpathSync.native(directory) !== leasedDirectory) {
      throw new CliError(
        'PROJECT_PATH_FORBIDDEN',
        `Buffer directory changed during lease acquisition: ${path.resolve(directory)}`
      );
    }
    return callback(leasedDirectory);
  });
}
...
function runGcWithHistorySnapshot(ctx, db, {
  ...
  if (!graceActive && !skipBufferFiles) {
    const bufferResult = withOptionalBufferDirectoryLease(bufsDir, () => {
