  ...
  try {
    return runOptimisticEvidenceMutation(db, {
      ...
      beforeMutate: (subject, evidenceByOwner) => {
        if (bufferPathGuard) assertBufferDirectoryPathUnchanged(bufferPathGuard);
        return observeClockSafetyInTransactionOrThrow(
          db,
...
function assertBufferDirectoryPathUnchanged(captured) {
  let resolvedRoot;
  try {
    resolvedRoot = fs.realpathSync.native(captured.requestedRoot);
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
    throw bufferDirectoryPathForbidden(
      'Project root changed during buffer GC',
      captured.requestedRoot
    );
  }
  if (resolvedRoot !== captured.canonicalRoot) {
    throw bufferDirectoryPathForbidden(
      'Project root changed during buffer GC',
      captured.requestedRoot
    );
  ...
  if (!captured.exists) {
    try {
      fs.lstatSync(captured.directory);
    } catch (error) {
...
function captureOptionalBufferDirectory(root, directoryName) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') {
      ...
      assertBufferDirectoryPathUnchanged(captured);
      return captured;
    }
    throw error;
  }
  ...
  return { requestedRoot, canonicalRoot, directory, components, exists: true };
}

function collectGuardedBufferEvidence(collector, captured) {
  const evidence = collector(captured.directory);
  assertBufferDirectoryPathUnchanged(captured);
  return evidence;
}

function withOptionalBufferDirectoryLease(captured, callback) {
  assertBufferDirectoryPathUnchanged(captured);
...
function runGc(ctx, db, {
  ...
  const bufferPathGuard = skipBufferFiles
    ? null
    : captureOptionalBufferDirectory(ctx.root, BUFS_DIR_NAME);
  ...
  const preparedBufferEvidence = !skipBufferFiles && collectBufferEvidenceNow
    ? collectGuardedBufferEvidence(collectBufferEvidenceNow, bufferPathGuard)
    : null;
  const bufferGcCutoffs = skipBufferFiles
    ? []
    : bufferDirectoryPaths(bufferPathGuard.directory).flatMap((file) => {
        try {
...
function runGcWithHistorySnapshot(ctx, db, {
  ...
  if (!graceActive && !skipBufferFiles) {
    const bufferResult = withOptionalBufferDirectoryLease(bufferPathGuard, (
      leasedBufsDir,
      assertDirectoryUnchanged
    ) => {
      ...
      const currentObservation = tx(db, () => {
        assertDirectoryUnchanged();
