...
export function runTmux(args, opts = {}) {
  const result = spawnSync('tmux', args, {
    ...
    input: opts.input || '',
    // xx-02: a hung tmux (D-state pane, wedged pty, stuck filesystem) must not
    // freeze the shared runtime event loop forever. Normal tmux commands finish
    // in milliseconds; a 5s bound only trips when tmux is genuinely stuck.
    timeout: opts.timeout ?? 5000
  });
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT' || result.killed) {
      throw new CliError('TMUX_ERROR', `tmux timed out after ${opts.timeout ?? 5000}ms: tmux ${args.join(' ')}`);
    }
...
export function tmuxInstallHint({
  ...
  if (platform === 'darwin') return 'Install tmux with: brew install tmux';
  if (platform === 'linux') {
    if (pathExists('/etc/debian_version')) return 'Install tmux with: sudo apt-get update && sudo apt-get install -y tmux';
    if (pathExists('/etc/alpine-release')) return 'Install tmux with: sudo apk add tmux';
    if (pathExists('/etc/arch-release')) return 'Install tmux with: sudo pacman -S --noconfirm tmux';
    if (pathExists('/etc/fedora-release') || pathExists('/etc/redhat-release')) {
      return 'Install tmux with: sudo dnf install -y tmux (or sudo yum install -y tmux on older systems)';
    }
    if (pathExists('/etc/SuSE-release')) return 'Install tmux with: sudo zypper --non-interactive install tmux';
    return 'Install tmux with your Linux system package manager (apt/apt-get, dnf/yum, apk, pacman, or zypper).';
  }
...
export function tmuxInstallPlan({
  ...
  if (!isRoot && !commandAvailable('sudo')) return null;
  ...
  if (commandAvailable('zypper')) {
    return { manager: 'zypper', commands: [`${sudo}zypper --non-interactive install tmux`] };
  }
  return null;
}
...
export function tryInstallTmux() {
  ...
  if (!plan) return { ok: false, output: 'no supported package manager with sufficient privileges found' };
...
export function tmuxListSessionNames() {
  const result = spawnSync('tmux', ['list-sessions', '-F', '#{session_name}'], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe']
  });
  // No tmux server (status 1, "no server running") or no sessions → empty list.
...
export function tmuxLaunchFingerprint(sessionName) {
  return tmuxSessionEnvironmentValue(sessionName, LAUNCH_FINGERPRINT_ENV);
}

export function tmuxProviderState(sessionName) {
  return tmuxSessionEnvironmentValue(sessionName, PROVIDER_STATE_ENV);
}

function managedProjectRoot(root) {
  try {
...
function legacyManagedProjectRoot(root) {
  return path.resolve(root);
}

export function tmuxManagedSessionPrefix(ctx) {
  return `hcc-${shortHash(managedProjectRoot(ctx.root))}-`;
}

export function tmuxManagedSessionName(ctx, peerId) {
  const name = `${tmuxManagedSessionPrefix(ctx)}${sanitizePeerPart(peerId, 'peer')}`;
  return name.slice(0, 80);
}

function legacyTmuxManagedSessionPrefix(ctx) {
  return `hcc-${shortHash(legacyManagedProjectRoot(ctx.root))}-`;
}

function legacyTmuxManagedSessionName(ctx, peerId) {
  return `${legacyTmuxManagedSessionPrefix(ctx)}${sanitizePeerPart(peerId, 'peer')}`.slice(0, 80);
}

function rootsIdentifySameProject(left, right) {
  try {
...
export function tmuxManagedSessionPrefixMatches(ctx, sessionName, sessionRoot = null) {
  if (!sessionName) return false;
  if (sessionName.startsWith(tmuxManagedSessionPrefix(ctx))) return true;
  return Boolean(sessionRoot) && rootsIdentifySameProject(ctx.root, sessionRoot) &&
    sessionName.startsWith(legacyTmuxManagedSessionPrefix({ root: sessionRoot }));
}

export function tmuxManagedSessionNameMatches(ctx, sessionName, peerId, sessionRoot = null) {
  if (!sessionName) return false;
  if (sessionName === tmuxManagedSessionName(ctx, peerId)) return true;
  return Boolean(sessionRoot) && rootsIdentifySameProject(ctx.root, sessionRoot) &&
    sessionName === legacyTmuxManagedSessionName({ root: sessionRoot }, peerId);
}

export function tmuxEnvironmentArgs(env) {
  const args = [];
...
export function tmuxPaneInfo(targetPane = null) {
  ...
  // NOTE: do not use tab characters as field separators — older tmux (e.g. 3.3a
  // on Debian) replaces whitespace in display-message/list-panes -F output with
  // '_', which silently corrupts parsing and produces invalid pane targets. '|'
  // is preserved across tmux versions and never appears in pane_id/pid/command/
  // dead; the path is placed last and rejoined so a '|' inside a path is safe.
  const format = '#{pane_id}|#{pane_dead}|#{pane_pid}|#{pane_current_command}|#{pane_current_path}';
