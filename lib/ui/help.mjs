...
Usage:
  ${cliName} web [--host HOST] [--port N] [--token TEXT] [--local] [--tls] [--trust-proxy] [--no-token] [--no-discover] [--no-guidance]
...
terminating TLS on the same machine, add --trust-proxy so same-origin checks use
X-Forwarded-Proto/Host. Forwarded headers are accepted only from a loopback peer.

The printed ?token= URL is used once: the browser exchanges it for an HttpOnly
session cookie and is redirected to a token-free URL, so the token no longer
sits in the address bar, browser history, or Referer headers. The CLI and the
regression API keep authenticating with ?token= or the Authorization header.
