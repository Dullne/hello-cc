...
Commands:
  ...
  gc [--older-than DAYS] [--history] [--yes] Clean technical state; opt in to history cleanup
  doctor                       Run a read-only database health check (integrity, sizes, row counts)
Internal:
...
Environment:
  ...
  HCC_RUNTIME_CA         CA file for HCC_RUNTIME_URL=https endpoints
`);
    },
...
Usage:
  ...
  ${cliName} task dispatch --to ID --title TEXT [--body TEXT] [--from ID] [--message TEXT] [--no-inject] [--force]
  ${cliName} task dispatch --to ID --id N [--from ID] [--message TEXT] [--no-inject] [--force]
  ...
  ${cliName} task takeover [--peer ID] --id N --reason TEXT [--policy any|blocked|stale|blocked-or-stale] [--force] [--stale-after SECONDS]
  ${cliName} task next [--peer ID] [--force] [--count N]
  ${cliName} task update [--peer ID] --id N --status STATUS [--summary TEXT] [--body TEXT] [--to ID]
  ${cliName} task running|review|blocked|abandoned [--peer ID] --id N [--summary TEXT] [--body TEXT] [--to ID]
...
task dispatch explicitly assigns a new or existing task to one peer, sends a
durable task message, and injects the startup prompt only when the peer has a
running managed Claude/Codex terminal. Use --no-inject for message-only
dispatch; use --force only when intentionally injecting while the target already
owns another active task.
...
The default policy is blocked-or-stale: takeover is allowed only when the task
is blocked or its owner is stale/exited, so an active owner's task is not taken
silently. Use --force (or --policy any) to take over an active owner's task
explicitly; use --policy blocked or --stale-after to narrow the precondition.
task running, task review, task blocked, and task abandoned are shortcuts for
...
Usage:
  ${cliName} gc [--older-than DAYS] [--history] [--yes]

Examples:
  ${cliName} gc
  ${cliName} gc --older-than 14 --yes
  ${cliName} gc --older-than 14 --history --yes

Clean stale technical state for the current project: dead peers, expired locks,
and old buffer files. Events, completed tasks, messages, and eligible handoffs
are retained unless --history is supplied.
Handoffs linked to open tasks are always preserved. By default this is a dry-run.
Technical deletion requires --yes, and
history deletion requires both --history and --yes.
--older-than must be zero or greater. During clock grace,
every age-based category is deferred.
`);
    },
...
Claude/Codex hooks, claude/codex shims, and the shell PATH entry for shims.
...
`);
    ...
    helpInstallHooks() {
      console.log(`${cliName} install-hooks

Usage:
  ${cliName} install-hooks
  ${cliName} install-hooks --status
  ${cliName} install-hooks --uninstall

Installs, checks, or removes Claude Code and Codex hook entries used by
hello-cc. This is a maintenance command; hcc up, hcc web, and hcc setup install
hooks automatically when needed.
`);
    },

    helpShim() {
      console.log(`${cliName} shim

Usage:
  ${cliName} shim install
  ${cliName} shim status
  ${cliName} shim uninstall

Installs, checks, or removes claude/codex shims in ~/.hcc-shims and manages the
shell PATH entry for that directory. hcc web and hcc setup install shims
automatically when provider CLIs are available on PATH.
`);
    },
...
Usage:
  ${cliName} web [--host HOST] [--port N] [--token TEXT] [--local] [--tls] [--trust-proxy --proxy-origin ORIGIN] [--no-token] [--no-discover] [--no-guidance]
...
By default, the web runtime listens on 0.0.0.0 and generates a token scoped to
that runtime. Use HCC_WEB_TOKEN or --token to set an explicit token, --local to
bind only to 127.0.0.1, or --no-token only for a trusted
local/test environment. The web console exposes a writable terminal, so binding
a non-loopback address prints an exposure warning, and --no-token is refused on
any non-loopback bind (use --local with it). Prefer --local plus an SSH tunnel
(ssh -L 8787:127.0.0.1:8787) or a TLS reverse proxy for remote access. Use --tls
to serve over HTTPS/WSS with an auto-generated self-signed certificate (stored
under ~/.hello-cc/tls, reused across restarts); this encrypts the token and
terminal stream against passive LAN sniffing. The browser will warn about the
self-signed certificate until you trust it manually. For an HTTPS reverse proxy
terminating TLS on the same machine, use --trust-proxy together with a fixed
--proxy-origin https://host[:port]. Forwarded scheme and host must match that
origin and arrive from loopback. For HCC_RUNTIME_URL=https endpoints with a
private CA, set HCC_RUNTIME_CA to the CA file.

The printed ?token= URL is the one-time entry: the browser exchanges it for an
HttpOnly session cookie and is redirected to a token-free URL. After that,
subsequent requests and the address bar no longer carry the token (the initial
URL remains in browser history as the access link). CLI API requests use the
Authorization header; query tokens are accepted only by the browser exchange.

Version 1.0.0 uses schema v7 with a verified pre-migration backup and no
downgrade support, full-session provider peer IDs without legacy remapping, and
Runtime API v2. Liveness follows process evidence; only unknown evidence gets a
120-second grace. hcc gc keeps history unless --history is explicit.

Accepted risks: the default 0.0.0.0 listener is plaintext HTTP for trusted LANs,
and an authenticated browser may select any existing server directory.
