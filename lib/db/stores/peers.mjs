...
function crossesV1ProviderIdentityBoundary(binding, conflict) {
  const providerSession = bindingProviderSessionValue(binding);
  if (!providerSession || !binding?.provider) return false;
  const v1Peer = providerSessionPeerId(binding.provider, providerSession);
  return binding.peer === v1Peer && conflict?.peer !== v1Peer;
}

function withoutProviderSession(binding) {
  return {
    ...binding,
...
export function createPeerBindingStore(deps = {}) {
  ...
  function canonicalizePeerBinding(db, binding, options = {}) {
    ...
    const preV1Conflict = crossesV1ProviderIdentityBoundary(next, conflict);
    const passiveObservation = !incomingRuntime || next.resume_mode === 'detected';

    if (preV1Conflict && passiveObservation) {
      next = withoutProviderSession(next);
      return { peer: next.peer, binding: next, merged_from: null };
    }

    if (preV1Conflict && incomingRuntime && !override) {
