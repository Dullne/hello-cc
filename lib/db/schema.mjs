...
function failedRollbackMetadata(db, context) {
  let transactionActive = null;
  try {
    transactionActive = Boolean(db.isTransaction);
  } catch {}
  return {
    context,
    transactionActive,
  ...
  try {
    ...
    try {
      ...
      throw aggregateCleanupFailure(
        ...
        failedRollbackMetadata(db, 'Write transaction rollback')
      );
    }
    throw err;
  }
...
// Deferred read transaction: gives a consistent snapshot for multi-table reads
// without taking a write lock (unlike tx's BEGIN IMMEDIATE). Use for read-only
// snapshot assembly so peers/tasks/locks counts cannot drift mid-read (conc-06).
export function readTx(db, fn, options = {}) {
  db.exec('BEGIN;');
  try {
    const result = fn();
    db.exec('COMMIT;');
    ...
    try {
      ...
      throw aggregateCleanupFailure(
        err,
        cleanupError,
        `${options.cleanupContext || 'Read transaction'} failed and rollback also failed`,
        failedRollbackMetadata(
          db,
          `${options.cleanupContext || 'Read transaction'} rollback`
        )
      );
    }
    throw err;
...
export function initSchema(db, {
  ...
  `).get());
  if (existingVersion < DB_SCHEMA_VERSION &&
      (existingVersion > 0 || hasExistingSchema) &&
      typeof beforeMigration === 'function') {
    beforeMigration({ fromVersion: existingVersion, toVersion: DB_SCHEMA_VERSION });
  }
  createBaseSchema(db);
...
function runSchemaMigrations(db) {
  const migrations = [
    ...
    {
      version: 6,
      name: 'persist lock ttl',
      up(database) {
        const hadTtlColumn = tableColumns(database, 'locks').has('ttl_sec');
        addColumnIfMissing(database, 'locks', 'ttl_sec', 'ttl_sec INTEGER NOT NULL DEFAULT 900');
        database.exec(hadTtlColumn
          ? `
              UPDATE locks
              SET ttl_sec = 900
          ...
          : `
              UPDATE locks
              SET ttl_sec = CASE
                WHEN expires_at > created_at THEN MIN(expires_at - created_at, 3600)
                ELSE 900
              END;
            `);
      }
    ...
    {
      version: 7,
      name: 'peer process identity fingerprints',
      up(database) {
        addColumnIfMissing(database, 'peers', 'pid_start_token', 'pid_start_token TEXT');
        addColumnIfMissing(database, 'peers', 'pid_command_hash', 'pid_command_hash TEXT');
      }
    }
  ];
  ...
  // Already fully migrated: skip the write transaction entirely. Read-only
  // commands (hcc status/peers/task list) then open the DB without taking a
  // RESERVED write lock or appending WAL frames on every connect (conc-01).
  if (version === DB_SCHEMA_VERSION && migrations.every((m) => applied.has(m.version))) {
    return;
  }
  tx(db, () => {
