...
> **最终处置状态(2026-08-04):** 本文以下缺陷、代码行号和轮次说明是历史审查快照，不再代表当前树。本文跟踪的已知可操作正确性、安全性和可靠性问题均已修复；目前没有遗留的已知可操作缺陷。
>
> 明确保留两项用户接受的部署风险：(1) 默认继续支持受信任内网的 `0.0.0.0` 明文 HTTP 访问；(2) 已认证浏览器继续可以输入任意服务器上已存在的目录，不增加根目录 allowlist。完全缺少身份/产品字段的旧 runtime pointer 仍可读取，但 v1 新指针带完整进程身份，未知身份的破坏性处理受 120 秒窗口和严格复核约束。`bin/hcc.mjs` 的模块拆分属于架构债务，不作为未修复缺陷计数。
>
